...
export default function Home() {
  ...
  // ── ALL values guaranteed primitive via String()/typeof checks ──
  const flags = profile?.active_flags || {};
  const armFeel = typeof flags.current_arm_feel === 'number' ? flags.current_arm_feel : null;
  const dso = typeof flags.days_since_outing === 'number' ? flags.days_since_outing : 0;
    ...
    <div style={{ paddingBottom: 20 }}>
      ...
      <div style={{ background: 'var(--color-maroon)', padding: '14px 16px 0' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
          <div>
            <div style={{ fontSize: 9, color: 'var(--color-rose-blush)', letterSpacing: '0.06em' }}>{'UChicago Baseball'}</div>
        ...
        <div style={{ borderTop: '0.5px solid rgba(255,255,255,0.12)', padding: '8px 0 12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', gap: 16 }}>
            ...
            {totalEx > 0 && <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.5)' }}>{String(totalEx)}{' exercises'}{estDuration != null ? ` \u00B7 ~${String(estDuration)}m` : ''}</span>}
          </div>
      ...
      {!isNewPitcher && !hasCheckedIn && (
        <div onClick={() => navigate('/coach')} style={{ margin: '8px 12px 0', border: '1.5px solid var(--color-rose-blush)', background: 'rgba(92,16,32,0.05)', borderRadius: 12, padding: '10px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' }}>
          <div>
            <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--color-ink-primary)' }}>{'Morning check-in'}</div>
            <div style={{ fontSize: 10, color: 'var(--color-ink-muted)', marginTop: 2 }}>{'Check in to get today\'s personalized plan'}</div>
          </div>
          <div style={{ background: 'var(--color-maroon)', borderRadius: 8, padding: '6px 14px', fontSize: 11, fontWeight: 700, color: '#fff' }}>{'Check In'}</div>
        </div>
      )}
      ...
      <div style={{ padding: '0 12px' }}>
        {isNewPitcher ? (
          <div style={{ background: '#fff', borderRadius: 12, padding: 16, textAlign: 'center', marginTop: 12 }}>
            ...
            <p style={{ fontSize: 12, color: 'var(--color-ink-muted)' }}>{'Head to the Coach tab to check in or log an outing to get your first plan.'}</p>
          </div>
        ) : (
          <>
            {isViewingPast && (
              <div style={{ textAlign: 'center', marginTop: 8 }}>
                <button onClick={() => setSelectedDate(null)} style={{ padding: '4px 14px', fontSize: 11, fontWeight: 600, background: 'var(--color-maroon)', color: '#fff', border: 'none', borderRadius: 14, cursor: 'pointer' }}>{'Back to today'}</button>
              </div>
            )}

            {/* ── Brief card ── */}
            {(morningBrief || !todayEntry) && (
              <div style={{ background: '#fdf8f8', borderLeft: '3px solid var(--color-maroon)', borderRadius: '0 10px 10px 0', padding: '10px 14px', marginTop: 12 }}>
                ...
                {morningBrief && <p style={{ fontSize: 11, color: 'var(--color-ink-secondary)', lineHeight: 1.6, fontStyle: 'italic', margin: 0 }}>{morningBrief}</p>}
                {todayEntry && (
...
function StatCol({ label, value }) {
  return (
    <div style={{ flex: 1, textAlign: 'center' }}>
      <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--color-ink-primary)' }}>{String(value)}</div>
      <div style={{ fontSize: 8, color: 'var(--color-ink-muted)', marginTop: 1, textTransform: 'uppercase' }}>{String(label)}</div>
    </div>
  );
}
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <div style={{ display: 'flex', gap: 4 }}>
        {[...Array(7)].map((_, i) => <div key={i} style={{ flex: 1, height: 50, background: 'var(--color-cream-border)', borderRadius: 7 }} />)}
      </div>
