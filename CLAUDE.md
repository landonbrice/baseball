...
2. **Pre-existing bug: `db.list_program_holds_for_date`** — Discovered during Plan 7 C2 review. Queries non-existent `pitcher_id`/`event_date` columns on `program_hold_events` (real columns are `program_id` + `hold_date`). Mini-app `/api/programs/holds-today` endpoint silently returns empty in production → B2's amber "Program paused today" pill never renders. Plan 7's new `list_program_holds_for_pitcher` is correct (joins through `programs.pitcher_id`). Fix is a separate hotfix.
