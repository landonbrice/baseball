...
> Sprint status: Phases 1-20.1 + Sprint 0.5 + Tier 1 Hardening + Redesign Spec 1 + Redesign Spec 2 + Check-in Hotfix + Hotfix #2 complete. Tier 1 landed coach sidebar team-name enrichment, exercise-name hydration at write, canonical morning_brief shape, and snapshot cache for exercises. Spec 1 replaced the generic Tailwind shell with the editorial brand system. Spec 2 replaced the Team Overview body with a triage feed — Scoreboard anchor + dynamic TeamLede + HeroCard (flagged) + CompactCard (on-track) + PendingStrip + 90s visibility-gated auto-refresh + editorial PlayerSlideOver redesign (Today/Week/History tabs all rebuilt). Check-in Hotfix closed morning_brief JSON leaks into chat + saved-plan subtitles, threaded `energy` through `/chat` backend, and migrated frontend quickClassify to 1-10 scale. Hotfix #2 ([PR #2](https://github.com/landonbrice/baseball/pull/2)) closed the last Sprint 0.5 drift: outing post-pitching button row migrated 1-5 → 1-10 (matches `PostThrowFeel.jsx` pattern), quickClassify thresholds tuned (great/perfect → 9 not 10, good/fine → 7 not 8, terrible → 3 not 2; recovery ack `feel >= 7` not `>= 8`), and the `parseInt` 1-10 clamp restored (D4's extraction had dropped numeric input handling). Next: Spec 3 (deferred slide-overs / nudge endpoint / polish), remaining pages (Schedule / Team Programs / Phases / Insights), Tier 2, The Ledger, weight logging UI, mini-app energy-capture UI step, Regression #2 (Telegram getUpdates conflict — infra).
