...
| Plan 8 (PB) | Program Builder v1 — Cleanup + Roster Rollout | 05-15 | Closes the v1 cleanup carryovers from Plan 7 + rolls program-aware plan-gen to the full roster behind a per-pitcher kill-switch. **A1 audit script** (`pitcher_program_app/scripts/verify_saved_plans_deprecated.py`): Supabase read-only query of `saved_plans.created_at >= 2026-04-30` (when Plan 7 A15 soft-deprecation logging landed); exit codes 0 (zero writes — safe to drop) / 1 (writes found — caller still active) / 2 (connection failure). This is the gating signal for B1. **A2 structured logging + per-pitcher kill-switch**: `program_aware_compose` log event at the end of `compose_program_aware_plan` in `program_aware_planner.py` + matching event for legacy fall-through in `_select_plan_path` (sentinel `plan_source="legacy_path_selected"`). New `bot/services/db.py::set_feature_flag(pitcher_id, key, value)` read-modify-write helper. `POST /admin/program-flag/{pitcher_id}/{on|off}` route gated by the same `X-Guardian-Admin-Token` shared-secret header pattern (unset → 503, wrong → 401); best-effort audit via a `system_observations` sentinel row. **Deviation (a)**: plan spec assumed `compose_program_aware_plan` took a `program` dict — real signature takes `throwing_rx`/`lifting_rx` separately, so A2 threaded `program_id` as an optional kwarg + computed `domain` locally. **Deviation (b)**: `system_observations` columns are `event_type`/`severity_hint`/`metadata`/`source`/`service`/`surface`/`route_or_job` not the spec's `category`/`severity`/`signature` — A2 adapted the audit payload accordingly. **B2 canonical phase keys**: migration `031_template_phase_keys.sql` adds `training_phase_blocks.template_phase_keys text[]` and idempotently backfills via regex/emphasis fallback (all 5 UChicago rows backfilled). `phaseToTemplatePhaseIds()` client-side bridge helper deleted; coach-app `PhaseTimeline` reads the column directly. **C1 drift insight Archive/Accept CTAs**: migration `032_coach_suggestions_accepted_at.sql` adds `accepted_at timestamptz`. 14-day accept-suppression layered onto `suggestion_exists_for_today` (today-wins short-circuit preserved by a deliberate refactor). `POST /api/coach/insights/{id}/action` accepts `accept`|`dismiss`. `InsightCard` Archive button archives the program via the existing endpoint then dismisses the insight; Accept button writes `accepted_at`. **C2 A4 insight body LLM polish**: `polish_insight_body(suggestion)` async with per-category prompt map (`drift` / `mismatch` / `team_lagging`). Polish wired into the 3 insertion sites in `_generate_coach_insights_for_team` **after dedup, before insert**. `compute_daily_digest` migrated sync → async; `existing_health.py::_run_sync_digest` updated to drive the async digest via `asyncio.run` inside `asyncio.to_thread` (preserves sync monkey-patches in existing tests via `inspect.iscoroutine` branch). Unknown category → passthrough (no LLM call); LLM error/timeout/empty → rule-based body unchanged. Mutation-safe (`out = dict(suggestion)` then `pa = dict(out.get("proposed_action") or {})`); stamps `proposed_action.polished=True` only on success. **Deviation (c)**: C2 had to migrate `compute_daily_digest` to async to call `polish_insight_body` — 4 caller sites touched including the Guardian `existing_health` collector. **C3 coach attaches research docs**: `GET /api/coach/research-docs` lists docs from frontmatter via reuse of `_load_index()` from `research_resolver` (deviation (f) — actual function name, not the spec's `_list_all_docs()`). `PATCH /api/coach/block-library/{template_id}/research-docs` updates `block_library.research_doc_ids`. New `TemplateResearchEditor` two-column attach/detach modal (Available ↔ Attached) in `coach-app/src/components/team-programs/`, wired into the Library Edit Research flow; brand-token-compliant (`bg-bone`, `text-charcoal`, `bg-maroon`, `hover:bg-maroon-ink`). **Deviation (d)**: C3 vitest `vi.mock` factories cannot reference outer variables due to hoisting — switched to bare `vi.fn()` pattern. **Deviation (e)**: Tailwind tokens — `text-meta` not `text-body-xs`, `hover:bg-maroon-ink` not `bg-maroon-dark`. **D1 full-roster enablement**: `scripts/enable_program_aware_for_team.py` (118 lines, idempotent, `--dry-run` + `--revert`, per-pitcher status output, exit codes 0/1/2 mirroring A1). Flips `program_aware_plan_gen=true` for all 12 UChicago pitchers (and `test_pitcher_001`). **D2 program-aware canary metric**: `compute_program_aware_canary(days=7)` sync helper in `health_monitor.py` — defensive try/except both around `list_pitchers()` and the `daily_entries` query so it never breaks the digest. 17-line render block in `format_digest_message` inserted **BEFORE** the Guardian summary section. Thresholds: `<0.6` 🚨 CRITICAL, `<0.8` ⚠️ regressing. 4 tests cover zero-flagged short-circuit / all-prescribed / mixed sources / missing `plan_generated` field. **B1 (`saved_plans` hard-drop) deferred** to a small follow-up PR gated on A1 returning exit 0 against prod Supabase. **Migrations** applied via Supabase MCP: `031_template_phase_keys.sql`, `032_coach_suggestions_accepted_at.sql`. **Env vars**: no new ones (A2 admin endpoint reuses `GUARDIAN_ADMIN_TOKEN`). **Tests**: 876 backend / 68 mini-app / 195 coach-app — all green. **Tag**: `program-builder-v1-cleanup-complete` is **NOT pushed** by this PR — it waits on a 7-day D2 canary monitoring window post-merge. If the canary holds ≥80% for 7 consecutive days, tag and push; if it dips, dispatch a fix on a follow-up branch. |
...
> **★ Program Engine re-architecture (in design — 2026-05-31).** A from-scratch re-think of program generation: **LLM-forward generation + deterministic guardrails + living (editable) knowledge**, with the **multi-week program as the primary artifact** that *projects* into days. Built clean in a **separate worktree** (engine module reading the good substrate, owing nothing to the slop pipeline). v1 = **clean-room demo on `landon_brice`** (goal = velocity, lifting+throwing unified); goal-agnostic engine underneath. **Design spec:** `docs/superpowers/specs/2026-05-31-program-engine-design.md` · **Recon evidence (6-front, file:line refs):** `docs/superpowers/research/2026-05-31-program-engine-recon.md`. Status: **spec drafted, awaiting green light to write the implementation plan** (open: "the drive" = program→day projection policy). Relates to / should reconcile with the existing Program Builder (item 2 below) before implementing.

1. **`saved_plans` hard-drop follow-up PR** — Immediate Plan 8 follow-up (not Plan 9 backlog). Gated on A1's audit script (`pitcher_program_app/scripts/verify_saved_plans_deprecated.py`) returning exit 0 against prod Supabase. Once clean: drop the `saved_plans` table + remove the dead `insert_saved_plan` / `get_saved_plans` / `get_saved_plan` paths across `api/routes.py`, `bot/services/db.py`, and mini-app `App.jsx` / `api.js` / `pages/PlanDetail.jsx`. Single small PR.
2. **Plan 9 — Program Builder v2 backlog** — Prohibited-day conflict logic (declarative `prohibited_throw_kinds` template field + 409 + `?confirm=true` retry — deferred from Plan 5 / Plan 6 A5, still open); cross-pitcher favorite copy ("I want what Reed did Tuesday"); brand-shell workspace package (`packages/shell`); coach-authored *new* research docs (Plan 8 C3 shipped attach-existing; v2 adds authoring + frontmatter validation + git/Supabase choice); `pitcher_training_model.active_program_id` FK drop (depends on the orphan-table cleanup decision in Known Issues); WHOOP token rotation (post-2026-04-30 lockdown action item — Kamat / Kwinter / Richert).
3. **Baselines backfill / cold-start UX** — Pitchers without a baseline snapshot silently fall through to the legacy flat-trigger path. Decide: auto-seed a baseline after N check-ins vs. require coach to trigger, and how to indicate this in the coach UI.
4. **Tier 2 hardening** — Continuation of the Tier 1 sprint (not yet scoped — open question).
5. **The Ledger** — Modification history timeline on Profile. Data exists in `plan_generated.modifications_applied` + `pitcher_training_model.recent_swap_history`.
6. **Weight logging UI** — `working_weights` column exists, no UI. Unblocks exercise progression curves.
7. **Exercise progression curves** — Volume/intensity trends for key lifts over time. Blocked on weight logging.
8. **Inline coach panel** — Coach button on lifting block for in-context refinement without navigating to Coach tab.
