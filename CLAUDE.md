...
> Sprint status: Phases 1-20.1 + Sprint 0.5 + Tier 1 Hardening + Redesign Specs 1/2/3 + Check-in Hotfix + Hotfix #2 + **Phase 1 Trajectory-Aware Triage (absorbs Sprint 1a)** + Energy Capture UI + Team Daily Status Sync + **WHOOP RLS Lockdown + LLM Enrichment Recovery + Public Schema Lockdown** + **Audit-Driven Full Public-Schema Lockdown (05-08)** complete. Spec 3 rebuilt the remaining four coach pages (Insights / Schedule / Team Programs / Phases) on the Spec 2 Scoreboard-anchored editorial pattern and wired `POST /api/coach/pitcher/{id}/nudge` end-to-end (coach_actions migration, `send_nudge` service, PendingStrip flipped to live). Team Daily Status Sync centralized check-in/plan/work/team/date semantics in `bot/services/team_daily_status.py`, made `/api/staff/pulse?team_id=...` team-aware, refactored coach overview and staff pulse onto the shared service, cleaned frontend naming around `checkin_status`, and hardened Supabase selects against optional-column drift. WHOOP RLS Lockdown (migration `010_whoop_rls_lockdown.sql`) revoked anon/authenticated grants and enabled+forced RLS on `whoop_tokens`, `whoop_daily`, `whoop_pending_auth` (pre-state: full CRUD via the project anon key — tokens are now compromised and must be rotated; see Known Issues). LLM Enrichment Recovery: telemetry showed 23/25 plan-gen calls timing out at the 20s ceiling and the research injection pinning at 12kB on every call, producing ~80% silent `python_fallback` plans — fixed by `LLM_REVIEW_TIMEOUT` 20s→45s + `resolve_research(..., max_chars=4000)` in `plan_generator.py`, plus a new `compute_plan_health_rolling(days=7)` in `health_monitor.py` wired into the 9am digest with a <60% threshold so this regression class surfaces in days, not weeks. **Audit-Driven Full Lockdown (05-08)**: code audit found that the Apr 30 lockdown regressed within a week — the May 1 "programs core" migration series introduced 7 tables with RLS disabled and full anon CRUD (`programs`, `favorited_blocks`, `program_builder_sessions`, `program_hold_events`, `program_schedule_revisions`, `program_generation_failures`, `coach_visible_override_events`), `advance_program_counter(uuid, jsonb, date)` shipped with mutable search_path, AND the 11 tables CLAUDE.md described as "low-risk USING(true) WARNs" actually had their `Service role full access` policy bound to `PUBLIC` rather than `service_role`, so anon could reach them via PostgREST despite the intent of 010/012. Migration `017_full_public_rls_lockdown.sql` revokes anon/authenticated grants, enables+forces RLS, and replaces permissive policies with `service_role`-bound canonical policy across all 18 affected tables; `018_advance_program_counter_search_path.sql` pins the function. Post-state verified: every targeted table has `rls_enabled=true, rls_forced=true, anon_grants=0`, single `service_role full access` policy bound to `service_role`; `get_advisors(security)` returns only the dashboard-toggle `auth_leaked_password_protection` WARN. Design for an intent-vs-live divergence scanner at `docs/supabase-divergence-scanner.md` — the structural fix so this regression class surfaces in the 9am digest, not via ad-hoc audits. Build deferred. Next: divergence scanner v1, coach-app Phase 1 surfacing (category scores on PlayerSlideOver + flagged-feed copy), Tier-2 reliability cleanup (dev commands at `bot/main.py:975-979`, `/health` `pitcher_count` leak at `api/main.py:73`, `.gitignore` gaps), Tier 2 hardening, The Ledger, weight logging UI, Regression #2 (Telegram getUpdates conflict — infra), Issue #2 (energy capture defaulting to 3 across recovery/low-arm/Telegram paths — backend default lies, deferred from this hotfix).
...
| Audit-Driven Full Public-Schema Lockdown | Security regression sweep + scanner design | 05-08 | Code audit on 05-08 found the Apr 30 lockdown had regressed within a week. Two regression classes verified via `pg_policies` + `information_schema.role_table_grants` + `get_advisors`: (1) the May 1 "programs core" migration series introduced **7 tables with RLS disabled and full anon CRUD** (`programs`, `favorited_blocks`, `program_builder_sessions`, `program_hold_events`, `program_schedule_revisions`, `program_generation_failures`, `coach_visible_override_events`) — two of them carry `session_id` columns and were flagged `sensitive_columns_exposed`. `advance_program_counter(uuid, jsonb, date)` shipped with mutable search_path, missed by 011. (2) The 11 older tables CLAUDE.md described as low-risk `USING(true)` WARNs (`chat_messages`, `daily_entries`, `exercises`, `injury_history`, `mobility_videos`, `mobility_weekly_rotation`, `pitcher_training_model`, `pitchers`, `saved_plans`, `templates`, `weekly_summaries`) actually had their `Service role full access` policy bound to `PUBLIC` (not `service_role`) AND retained anon/authenticated grants, so anon could reach them via PostgREST. Migration `017_full_public_rls_lockdown.sql` (DO-block over 18 tables: revokes grants, drops the three known permissive policy variants, enables+forces RLS, creates `service_role`-bound canonical policy) and `018_advance_program_counter_search_path.sql` (pins the function). Verified post-apply: 18/18 tables `rls_enabled=true, rls_forced=true, anon_grants=0, single service_role policy`; `advance_program_counter.proconfig = ['search_path=public, pg_temp']`; advisor lints reduced to only the dashboard-toggle `auth_leaked_password_protection` WARN. Frontend exposure verified clean: `coach-app/src/hooks/useCoachAuth.jsx` is the only `createClient` call and uses `auth` schema only; mini-app has no `createClient`. Design for an intent-vs-live divergence scanner committed at `docs/supabase-divergence-scanner.md` (intent.yaml ↔ live `pg_policies` / grants / function configs / advisor lints, default = service_role-only locked; runs from the 9am digest; v1 = read-only audit, no auto-fix). PR #13 merged. Three CLAUDE.md tech-debt items independently confirmed already fixed and removed from Known Issues: `context_manager.py:174` `str()` coercion, `morning_brief` consolidation through `normalize_brief()` / `_unwrap_morning_brief()`, `LLM_REVIEW_TIMEOUT = 45`. |
...
### What's Next
1. **Supabase divergence scanner v1** — Build per `docs/supabase-divergence-scanner.md`. Checked-in `supabase-intent.yaml` declares default posture (RLS enabled+forced, no anon grants, single `service_role full access` policy) plus per-table exceptions and pinned function search_paths. `scripts/supabase_audit/audit.py` reads live state via the existing Supabase MCP, diffs against intent, exits non-zero on drift. Wire into 9am digest. The structural fix that makes the 05-08 regression class detectable in days, not via ad-hoc audits.
2. **Tier-2 reliability cleanup** — `bot/main.py:975-979` still registers `/whooptest`, `/testnotify`, `/healthdigest`, `/testemergency` in production; `api/main.py:73` `/health` exposes `pitcher_count` unauthenticated; `.gitignore` (root + `pitcher_program_app/`) doesn't cover `graphify-out/`, `past_arm_programs/`, `Highlights/`, `Lifting/`, `bot_structure/`, `files/`. Each is small, all three in one PR.
3. **Coach-app Phase 1 surfacing** — Category scores (tissue/load/recovery) are computed and persisted but not yet displayed. PlayerSlideOver Today tab needs a 3-stat row; flagged-feed copy should cite the driving category. No engine change; purely frontend read-through.
...
10. **Persist `day_focus` in `plan_generated`** — Today it's derived at read time in `team_daily_status.py`. Moving the write into `plan_generator.py` removes a derivation hop and makes the field authoritative.
11. **Regression #2** — Telegram `getUpdates` conflict (infra).
...
- **WHOOP token rotation still pending (and exposure window now larger)** — Originally flagged after 010. The 05-08 audit found the exposure window extended further: the 11 "low-risk USING(true)" tables and 7 May-1 programs-core tables were also reachable via the anon key until 017 landed. `whoop_tokens.access_token` / `refresh_token` for Kamat, Kwinter, Richert must still be considered compromised. Action: revoke their grants in the WHOOP developer portal and have them re-link via `/whoop`. Anon publishable key rotation in Supabase dashboard is also defensible; the anon key is publicly extractable from the deployed coach-app JS by construction (Vite inlines `VITE_SUPABASE_ANON_KEY` at build), so the security model is now "anon key grants nothing on `public` schema" rather than "anon key is secret" — which is the correct end-state, but means *any* future regression that re-grants anon access to a `public` table immediately re-opens the same blast radius. The divergence scanner is the structural fix.
- **Migration history drift between repo and live DB** — `pitcher_program_app/scripts/migrations/` only contains files up through `018_*`, but `supabase_migrations.schema_migrations` carries 11 May-1 migrations (`009_programs_core` through `016_advance_program_counter_fn`) that were applied via the dashboard / direct MCP `apply_migration` and never tracked in the repo. Clashing leading numbers — file-level `010_whoop_rls_lockdown.sql` vs. live `010_block_library_extensions` — confused the 05-08 audit briefly. Going forward: every `apply_migration` call should also commit its SQL to `scripts/migrations/` with a number higher than the highest live version. The divergence scanner should flag any live migration version that has no matching repo file as a separate drift class.
- **Divergence scanner pending build** — Design at `docs/supabase-divergence-scanner.md`. Until built, Supabase posture regressions are caught by ad-hoc audit only. Stage 1 already has a candidate cadence (9am digest line) and trust boundary (read-only, exits non-zero on drift, no auto-fix in v1).
- **`.env.production` placeholders are checked in** — `mini-app/.env.production` (just `VITE_API_URL`) and `coach-app/.env.production` (`VITE_API_URL`, `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY=` empty). Real values are injected by Vercel at build, so no git leak — but the in-repo placeholders mean a future accidental commit of real keys would diff "normally" and slip past review. Move both to `.env.production.example` and add `.env.production` to `.gitignore`.
...
- **Repo bloat from untracked dev artifacts** — `graphify-out/` (852K), `past_arm_programs/*.xlsx` (4.1M), `Highlights/`, `Lifting/`, `bot_structure/`, `files/`, `ui-elevation-mockup.jsx`, root-level `scripts/` have leaked into commits. Neither root `.gitignore` nor `pitcher_program_app/.gitignore` covers them. Need proper `.gitignore` + `git rm --cached` pass.
...
- Dev commands (`/testnotify`, `/whooptest`, `/healthdigest`, `/testemergency`) still registered at `bot/main.py:975-979` — remove before broader rollout. Anyone in chat with the bot can fire them.
- **`/health` endpoint leaks `pitcher_count`** — `api/main.py:73` returns roster size unauthenticated. Mild info disclosure; either drop the field or gate the endpoint.
