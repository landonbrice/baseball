...
> Sprint status: Phases 1-20.1 + Sprint 0.5 + Tier 1 Hardening + Redesign Specs 1/2/3 + Check-in Hotfix + Hotfix #2 + **Phase 1 Trajectory-Aware Triage (absorbs Sprint 1a)** + Energy Capture UI + Team Daily Status Sync + **WHOOP RLS Lockdown + LLM Enrichment Recovery + Public Schema Lockdown** + **Program Builder v1 — Plans 1-5 + Plan 6 Phase A (pitcher backend complete)** complete. **Plan 6 split 05-13 into pitcher half (Plan 6) and coach half (Plan 7)** to ship pitcher value sooner. Phase A backend tag `program-builder-v1-backend-complete` lands: A1 wires the dormant Plan 4 program-aware fork live in `process_checkin` (`_select_plan_path` testable seam), A1.5 makes `plan_generated.day_focus` authoritative at write time (closes a long-standing read-time derivation hop), A2 ships `POST/GET/DELETE /api/favorites` (caller-supplied snapshot, self-source enforced, no dedup), A3-pitcher ships `GET /api/programs/{drafts,history,active}` (active is domain-keyed `{throwing, lifting}`; list endpoints trim `generated_schedule_json` to keep payloads tight), A5 ships `POST /api/programs/{id}/recompute` (server-reads scheduled_throws, no prohibited-day conflict check in v1 — players build whenever — and writes `coach_visible_override_events` on every schedule-changing recompute). Phase B (mini-app Programs tab rebuild, Home ribbon + per-block favorite hearts + held affordance, Builder slide-over) is next. Coach mirrors (`/api/coach/pitcher/{id}/{programs,drafts}`), insight types, and all coach-app UI deferred to Plan 7. Spec 3 rebuilt the remaining four coach pages (Insights / Schedule / Team Programs / Phases) on the Spec 2 Scoreboard-anchored editorial pattern and wired `POST /api/coach/pitcher/{id}/nudge` end-to-end. Team Daily Status Sync centralized check-in/plan/work/team/date semantics in `bot/services/team_daily_status.py`. WHOOP RLS Lockdown (migration `010_whoop_rls_lockdown.sql`) revoked anon/authenticated grants and enabled+forced RLS on `whoop_tokens`, `whoop_daily`, `whoop_pending_auth`. LLM Enrichment Recovery raised `LLM_REVIEW_TIMEOUT` 20s→45s + cut `resolve_research(...)` injection to 4kB; added rolling 7d enrichment metric to the 9am digest. Next: Plan 6 Phase B (Programs tab + Home ribbon + Builder slide-over), coach-app Phase 1 surfacing, System Guardian/Ops Intelligence spec integration, Tier 2 hardening, The Ledger, weight logging UI, Regression #2 (Telegram getUpdates conflict — infra), and Issue #2 (energy capture defaulting to 3 across recovery/low-arm/Telegram paths — backend default lies).
...
### What's Next
1. **Plan 6 Phase B — Mini-app UX** — Three pieces, builds on the backend tagged `program-builder-v1-backend-complete`. B1: rename `Plans.jsx` → `Programs.jsx`, rebuild as a single editorial scrolling page (Masthead → Today → Active Programs (≤2 cards, throwing + lifting from `/api/programs/active`) → Build a Program CTA → Drafts (from `/api/programs/drafts`) → Favorites (inline expansion, render-only snapshot) → History (from `/api/programs/history`) → Browse Templates). B2: Home additions — program ribbon above DailyCard when active, per-block Favorite heart button on each block header wired to `POST /api/favorites`, held affordance inline note when triage paused a program. B3: Builder slide-over with three states (Inputs form / Socratic chat / Preview); wires to the existing builder endpoints (`/builder/candidates`, `/builder/turn`, `/builder/finalize`, `/{program_id}/activate`, `/{program_id}/archive`). Uses `frontend-design` skill + existing brand shell (tokens.css, Masthead, Scoreboard, Lede, FlagPill, EditorialState). NOTE: brand shell is coach-app-only today — Phase B will either port the tokens into mini-app or stay with Telegram WebApp's CSS vars.
...
3. **Plan 7 — Coach half of Program Builder** — A3-coach mirrors (`/api/coach/pitcher/{id}/programs|drafts`), A4 (coach insight types: drift detection, mismatch flags, team-program completion roll-ups), B4 Team Overview additions (program strip per pitcher), B5 PlayerSlideOver Programs tab, B6 Team Programs rebuild, B7 build-entry-point selector (fold into B3 State A as coach sub-step), B8 Phases page templates column, B9 Insights UI wiring, B10 final tag `program-builder-v1-complete`.
