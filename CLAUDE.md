...
| Plan 1 (PB) | Program Builder v1 — Schema Foundation | 04-30 | Plan 1 of the Program Builder initiative. New tables (`programs`, `favorited_blocks`, `program_builder_sessions` + 4 operational), `block_library` extended for templates, `teams.training_phase` split per-domain (`throwing_phase`/`lifting_phase`), `pitcher_training_model` gains coach phase overrides + `feature_flags` jsonb, `saved_plans` backfilled to `favorited_blocks` (11 rows), Starter 7-day cadence reified as `block_library` template (`tpl_starter_7day_cadence_v1`), 4 current 7-day starters bootstrapped onto active programs. New module `bot/services/program_runtime.py` with `get_effective_phase` + `get_active_program_day`. Migrations 009-015 (+ 009b corrections). Task 0 (legacy plan_generator goldens) deferred to Plan 4 — see plan doc for findings. 340 backend tests pass. Tagged `program-builder-v1-foundation`. |
